...
router.post('/', jwChecks, restricted, (req, res) => {
  if (
    ...
    res
      ...
      .json({
        message:
          'In order to add a new Bid, you must provide a Project it belongs to , your price, the amount of time it will take, and if materials are included or not. '
      });
  } else {
    db('bids')
      .insert(req.body, 'id')
  ...
  db('bids')
    ...
    .then(bid => {
      if (bid) {
        ...
        res
          .status(404)
          .json({message: 'The Bid with the Specified ID does not exist'});
      }
    })
    .catch(err => {
      res
        ...
        .json({
          error: err,
          message: 'Unable to find the Specified Bid at this time.'
        });
    });
});
